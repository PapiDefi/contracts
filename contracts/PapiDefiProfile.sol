...
// File: @papidefi/papiswap-lib/contracts/token/BEP20/IBEP20.sol
...
// File: contracts/PapiDefiProfile.sol

pragma solidity ^0.6.0;

/** @title PapiDefiProfile.
@dev It is a contract for users to bind their address 
to a customizable profile by depositing a NFT.
*/
contract PapiDefiProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public papiToken;
    ...
    uint256 public numberPapiToReactivate;
    uint256 public numberPapiToRegister;
    uint256 public numberPapiToUpdate;
    ...
    constructor(
        IBEP20 _papiToken,
        uint256 _numberPapiToReactivate,
        uint256 _numberPapiToRegister,
        uint256 _numberPapiToUpdate
    ) public {
        papiToken = _papiToken;
        numberPapiToReactivate = _numberPapiToReactivate;
        numberPapiToRegister = _numberPapiToRegister;
        numberPapiToUpdate = _numberPapiToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends PAPI to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer PAPI tokens to this contract
        papiToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberPapiToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer PAPI token to this address
        papiToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberPapiToUpdate
    ...
    /**
     * @dev Claim PAPI to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        papiToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of PAPI to register
     * Callable only by owner admins.
     */
    function updateNumberPapi(
        uint256 newNumberPapiToReactivate,
        uint256 _newNumberPapiToRegister,
        uint256 _newNumberPapiToUpdate
    ) external onlyOwner {
        numberPapiToReactivate = newNumberPapiToReactivate;
        numberPapiToRegister = _newNumberPapiToRegister;
        numberPapiToUpdate = _newNumberPapiToUpdate;
    }
