...
// added functionnalities by the papidefi's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when PAPI mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 papiReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Stake PAPI tokens to TheBushV1
    ...
    // Withdraw PAPI tokens from STAKING.
